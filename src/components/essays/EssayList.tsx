...
type FilterStatus = 'all' | 'transcribed' | 'evaluated';
...
export default function EssayList({ essays, onDelete }: EssayListProps) {
  ...
  const filteredEssays = essays.filter((essay) => {
    ...
    if (filter === 'transcribed') {
      return essay.status === 'transcribed' || essay.status === 'evaluated';
    }
      ...
      <div className="flex flex-col sm:flex-row gap-4 items-start sm:items-center justify-between">
        ...
        <div className="flex flex-wrap gap-2">
          ...
          >
            ...
            {
              essays.filter(
                (e) => e.status === 'transcribed' || e.status === 'evaluated'
              ).length
            }
            )
          </button>
        ...
        <div className="flex items-center gap-2">
          ...
          <select
            ...
            className="px-3 py-2 border border-gray-300 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-indigo-500 text-gray-900"
          >
